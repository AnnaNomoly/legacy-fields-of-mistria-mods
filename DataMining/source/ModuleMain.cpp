...
// Full global dump: writes every top-level member of the GameMaker global instance to one
// markdown file. Stays minimal on purpose — depth cap (`DUMP_MAX_DEPTH`) is the only protection,
// `seen` is passed as nullptr so cycles are truncated by the depth cap rather than tracked,
// no SEH and no try/catch around the walk.
//
// The minimal form is load-bearing. A previous version wrapped each top-level member in a
// layered defense (SEH `__except`, C++ `try/catch`, per-member sidecar breadcrumb, fresh-vs-
// shared `seen`-map experiments) and consistently hard-crashed mid-walk — every crash
// bypassed every defensive layer. Under /EHsc, SEH unwinding skips C++ destructors, so each
// wrapped `ToMap`/`ToVector` leaked RValue copies whose underlying GML allocations didn't get
// released; hundreds of leaks later, GML's allocator was corrupt enough that some unrelated
// member's walk fast-failed the process. The defenses weren't catching crashes — they were
// causing them. Standalone per-member dumps (`WriteGlobalDump`) tolerate defenses fine
// because the destructor churn is bounded; the full-`global` walk does not. Don't add SEH /
// try-catch wrapping here.
