...
)
{
	...
	if (game_is_active && !GameIsPaused() && obj_dungeon_ladder_down_focused && Arguments[0]->ToInt64() == 6 && Result.ToBoolean() && !offering_chance_occurred && (floor_number < 19 || floor_number % 10 != 9))
	{
		...
		if (active_sigils.contains(Sigils::TEMPTATION) || roll < Config::config.offering_event_chance)
		{
			...
			if (resource == AriResources::STAMINA && ari_resource_to_value_map[AriResources::STAMINA] > Config::config.offering_stamina_requirement)
				PlayConversation("Conversations/Mods/Deep Dungeon/offering/stamina", Self, Other);
			if (resource == AriResources::MANA && ari_resource_to_value_map[AriResources::MANA] > Config::config.offering_mana_requirement)
	...
	else if (game_is_active && !GameIsPaused() && Config::config.disable_dungeon_lift && (ari_current_gm_room.contains("rm_mines") || ari_current_gm_room.contains("seal")) && obj_dungeon_elevator_focused && Arguments[0]->ToInt64() == 6 && Result.ToBoolean())
	{
