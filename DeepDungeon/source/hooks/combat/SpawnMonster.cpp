...
// MMAPI Monster::Hooks::BeforeMonsterSpawn callback.
void BeforeMonsterSpawn(MMAPI::Monster::SpawnMonsterContext& ctx)
{
	if (!active_traps.contains(Traps::LURING))
	{
		...
		if (active_sigils.contains(Sigils::ALTERATION) && sigil_of_alteration_monster_id != -1)
			ctx.SetMonster(static_cast<MMAPI::Monster::Ids>(sigil_of_alteration_monster_id));
	}

	const int monster_id = static_cast<int>(ctx.GetMonster());
	if (std::find(initial_floor_monsters.begin(), initial_floor_monsters.end(), monster_id) == initial_floor_monsters.end())
		initial_floor_monsters.push_back(monster_id);
}
