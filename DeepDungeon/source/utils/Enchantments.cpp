	...
	for (auto floor_trap = floor_trap_positions.begin(); floor_trap != floor_trap_positions.end();) {
		...
		if (distance <= 16)
		{
			...
			if (GetArmorSetBonuses().paladin.HallowedGround() && malfunction)
			{
				...
				MMAPI::Game::CreateNotification(true, MALFUNCTION_TRAP_NOTIFICATION_KEY);
			}
			else
			{
				...
				if (trap == Traps::CONFUSING)
				{
					...
					MMAPI::Game::CreateNotification(true, CONFUSING_TRAP_NOTIFICATION_KEY);
					MMAPI::Bark::Emit(MMAPI::Bark::Icons::QuestionMark, 0);
				...
				else if (trap == Traps::DISORIENTING)
				{
					...
					MMAPI::Game::CreateNotification(true, DISORIENTING_TRAP_NOTIFICATION_KEY);
					MMAPI::Bark::Emit(MMAPI::Bark::Icons::Mist, 0);
				...
				else if (trap == Traps::EXPLODING)
				{
					...
					MMAPI::Game::CreateNotification(true, EXPLODING_TRAP_NOTIFICATION_KEY);
					MMAPI::Bark::Emit(MMAPI::Bark::Icons::Angry, 0);
				}
				else if (trap == Traps::INHIBITING)
				{
					PlaySoundEffect("snd_bark_surprised", 100, 1);
					MMAPI::Game::CreateNotification(true, INHIBITING_TRAP_NOTIFICATION_KEY);
					MMAPI::Bark::Emit(MMAPI::Bark::Icons::NoCoin, 0);
				...
				else if (trap == Traps::LURING)
				{
					...
					MMAPI::Game::CreateNotification(true, LURING_TRAP_NOTIFICATION_KEY);
					MMAPI::Bark::Emit(MMAPI::Bark::Icons::Fishing, 0);

					// TODO: Restrict monster spawns as necessary (stalagmite_pink? TBD)
					std::vector<int> random_monsters;
					if (std::find(initial_floor_monsters.begin(), initial_floor_monsters.end(), static_cast<int>(MMAPI::Monster::Ids::Stalagmite)) != initial_floor_monsters.end())
						random_monsters = GenerateRandomMonstersIdsForCurrentFloor(Config::config.luring_trap_monster_spawn_count, static_cast<int>(MMAPI::Monster::Ids::Stalagmite));
					else if (std::find(initial_floor_monsters.begin(), initial_floor_monsters.end(), static_cast<int>(MMAPI::Monster::Ids::StalagmiteGreen)) != initial_floor_monsters.end())
						random_monsters = GenerateRandomMonstersIdsForCurrentFloor(Config::config.luring_trap_monster_spawn_count, static_cast<int>(MMAPI::Monster::Ids::StalagmiteGreen));
					else if (std::find(initial_floor_monsters.begin(), initial_floor_monsters.end(), static_cast<int>(MMAPI::Monster::Ids::StalagmitePurple)) != initial_floor_monsters.end())
						random_monsters = GenerateRandomMonstersIdsForCurrentFloor(Config::config.luring_trap_monster_spawn_count, static_cast<int>(MMAPI::Monster::Ids::StalagmitePurple));
					else
					...
					for (int i = 0; i < random_monsters.size(); i++)
						MMAPI::Monster::SpawnMonster(floor_trap->first + random_position_offset_distribution(random_generator), floor_trap->second + random_position_offset_distribution(random_generator), static_cast<MMAPI::Monster::Ids>(random_monsters[i]));
				...
				else if (trap == Traps::METEOR)
				{
					...
					if (instance_layer_exists)
					{
						...
						MMAPI::Game::CreateNotification(true, METEOR_TRAP_NOTIFICATION_KEY);
						MMAPI::Bark::Emit(MMAPI::Bark::Icons::ExclamationMark, 0);
				...
				else if (trap == Traps::GAZE)
				{
					...
					if (instance_layer_exists)
					{
						...
						MMAPI::Game::CreateNotification(true, GAZE_TRAP_NOTIFICATION_KEY);
						MMAPI::Bark::Emit(MMAPI::Bark::Icons::ExclamationMark, 0);
				...
				else if (trap == Traps::_VOID)
				{
					...
					if (instance_layer_exists)
					{
						...
						MMAPI::Game::CreateNotification(true, VOID_TRAP_NOTIFICATION_KEY);
						MMAPI::Bark::Emit(MMAPI::Bark::Icons::ExclamationMark, 0);
...
void ProcessCustomAOEs()
{
	for (CustomAOE& meteor : meteor_aoes)
	{
		if (meteor.is_active && current_time_in_seconds >= meteor.spawned_time + meteor.duration)
		{
			...
			double adjusted_health = MMAPI::Player::GetMaxHealth().ToDouble() * modifier;
			MMAPI::Player::ModifyHealth(-1 * adjusted_health);
...
void ProcessTreasureSpot(CInstance* Self, CInstance* Other)
{
	if (treasure_spot.state == TreasureSpot::WAITING_TO_SPAWN && treasure_spot.floors_to_descend == 0)
	{
		...
		if (floor_trap_positions.size() < biome_adjusted_max_traps_with_peril)
		{
			...
			if (instance_layer_exists)
			{
				...
				MMAPI::Game::CreateNotification(true, TREASURE_SPOT_SPAWNED_NOTIFICATION_KEY);
	...
	if (treasure_spot.state == TreasureSpot::SPAWNED)
	{
		...
		if (distance <= 8)
		{
			...
			MMAPI::Game::CreateNotification(true, TREASURE_SPOT_FOUND_NOTIFICATION_KEY);
			...
			//MMAPI::Item::Drop(greater_sigil_to_item_id_map[random_greater_sigil], ari_x, ari_y);

			if (floor_number < 20)
				MMAPI::Item::Drop(greater_sigil_to_item_id_map[GreaterSigils::SPIRIT_SURGE], ari_x, ari_y);
			else if (floor_number < 40)
				MMAPI::Item::Drop(greater_sigil_to_item_id_map[GreaterSigils::MEIKYO_SHISUI], ari_x, ari_y);
			else if (floor_number < 60)
				MMAPI::Item::Drop(greater_sigil_to_item_id_map[GreaterSigils::BENEDICTION], ari_x, ari_y);
			else if (floor_number < 80)
				MMAPI::Item::Drop(greater_sigil_to_item_id_map[GreaterSigils::CHAIN_SPELL], ari_x, ari_y);
			else if (floor_number < 100)
				MMAPI::Item::Drop(greater_sigil_to_item_id_map[GreaterSigils::ASTRAL_FLOW], ari_x, ari_y);
...
void ApplyOfferingPenalties(CInstance* Self, CInstance* Other)
{
	if (ari_resource_to_penalty_map[AriResources::HEALTH])
		MMAPI::Player::ModifyHealth(Config::config.offering_health_requirement * -1);
	if (ari_resource_to_penalty_map[AriResources::STAMINA])
		MMAPI::Player::ModifyStamina(Config::config.offering_stamina_requirement * -1);
	if (ari_resource_to_penalty_map[AriResources::MANA])
		MMAPI::Player::ModifyMana(Config::config.offering_mana_requirement * -1);
...
void TickTimeStoppedSystems(CInstance* Self, CInstance* Other, int64_t tick_delta)
{
	...
	for (int id = 0; id < MMAPI::StatusEffect::IdCount; ++id)
	{
		const auto status_id = static_cast<MMAPI::StatusEffect::Ids>(id);
		...
		if (status_id == MMAPI::StatusEffect::Ids::Restorative || status_id == MMAPI::StatusEffect::Ids::ShrineBoon)
		{
			if (last_tick >= last_update + 600)
			{
				StructVariableSet(effect, "last_update", last_tick);
				MMAPI::Player::ModifyHealth(5);
				MMAPI::Player::ModifyStamina(5);
			}
		}

		if (last_tick >= finish)
		{
			if (status_id == MMAPI::StatusEffect::Ids::FlameBreath)
				SetFireBreathTime(0);
			MMAPI::StatusEffect::Cancel(status_id);
		}
